...
用于 GE DSA 等血管造影设备的 DICOM 图像接收、查看、编辑和转发。支持 SCP 被动接收、本地文件导入、PACS 查询检索，以及从 DSA 工作站主动拉取图像。
...
- **PACS 查询与发送**：C-FIND 查询、C-STORE 发送
...
│          │                              │ └───────────────────┘ │
├──────────┴──────────────────────────────┴───────────────────────┤
│  [应用拆分并发送到 PACS]  [应用拆分并导出到本地]  目标患者: xxx   │
├─────────────────────────────────────────────────────────────────┤
│  状态栏                                                          │
└─────────────────────────────────────────────────────────────────┘
```
...
2. 点击 **应用拆分并发送到 PACS**：覆写患者信息 + 重新生成 UID → C-STORE 发送到 PACS
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| PACS AE Title | `PACS` | PACS 服务器的 AE Title |
| PACS 主机 | `127.0.0.1` | PACS 服务器 IP 地址 |
| PACS 端口 | `11112` | PACS 服务器端口 |
